...
**Lex 4.4.1 in the Ecosystem 3.1 train**.
...
The 4.4.1 patch preserves bounded continuity by deferring oversized provenance and
marks clipped text fields. See [4.4.1 release and recovery notes](docs/releases/lex-4.4.1.md).

Version 4.4.0 adds the narrow public `@smartergpt/lex/context` surface and fixes
canonical/deprecated MCP help names (#852). The new public API requires a minor
version. See [4.4 release notes](docs/releases/lex-4.4.md) and the
[migration guide](docs/releases/lex-4.4-migration.md). The verified public 4.3.0
package pair remains the release recovery baseline. This release does not activate
a scoped host or alter the installed-runtime verification boundary. Consumer
adoption and end-to-end startup qualification remain part of #854 follow-through.

For Lex 4.4.1, these values must agree:

- `package.json` and the package-lock root: `@smartergpt/lex@4.4.1`;
- `server.json`: `dev.smartergpt/lex@4.4.1`, transporting
  `@smartergpt/lex-mcp@4.4.1`;
- Ecosystem 3.1 manifest Lex and Lex-MCP targets: `4.4.1`;
- README and changelog current release: `4.4.1`;
...
`npm run check:release-drift` is a post-tag audit. It is expected to report the missing `v4.4.1`
...
remote main, exact public npm integrity, and the public Lex-MCP 4.4.1 dependency edge. Release tags
are protected against update and deletion by the active repository tag ruleset. The lane has no npm
publication authority.

The workflow publishes the exact retained `smartergpt-lex-4.4.1.tgz` with provenance. It never
repacks, and recovery continues only when an existing public 4.4.1 integrity exactly equals the
...
$public = npm view @smartergpt/lex@4.4.1 version engines dist.integrity --json |
  ConvertFrom-Json
...
1. refresh and verify each dependent lock from public `@smartergpt/lex@4.4.1`;
2. complete the manifest-selected LexSona, LexRunner, AXF, and STFC-Mod proofs;
3. publish exact `@smartergpt/lex-mcp@4.4.1` from its own reviewed checkout;
4. verify both public npm artifacts and their exact dependency edge;
5. create, verify, and push the signed Lex `v4.4.1` tag;
6. create, verify, and push the signed Lex-MCP `v4.4.1` tag;
