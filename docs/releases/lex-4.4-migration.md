...
Node 24 or newer remains required. Use the exact verified Lex/Lex-MCP 4.4.0 pair
...
Use `@smartergpt/lex-mcp@4.4.0` only after the exact pair is verified; preserve explicit
...
The exact core package is `@smartergpt/lex@4.4.0`. An explicitly qualified host may use:
...
| A dependent lock still resolves Lex 3.0.1 or another version | Refresh it from exact verified public Lex 4.4.0 bytes; reject directory links or mismatched integrity as release evidence. |
